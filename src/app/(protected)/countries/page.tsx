...
export default function CountriesPage() {
  ...
  function openEditModal(country: Country): void {
    setForm({ id: country.id, name: country.name });
    setIsEditing(true);
    setFormError(undefined);
    setIsModalOpen(true);
  }
  ...
  async function handleDelete(country: Country): Promise<void> {
    if (!token) return;

    const result = await Swal.fire({
      title: t('confirmDelete') ?? 'Are you sure?',
      text:
        t('confirmDeleteCountryText') ??
    ...
    try {
      setSaving(true);
      const res = await fetch(`${API_BASE}/api/Countries/${country.id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      await Swal.fire({
        icon: 'success',
        title: t('countryDeleted') ?? 'Country deleted.',
        timer: 1500,
      ...
      console.error(err);
      setFormError('Failed to delete country');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      const url = isEdit
        ? `${API_BASE}/api/Countries/${form.id as number}`
      ...
      await Swal.fire({
        icon: 'success',
        title: isEdit
          ? t('countryUpdated') ?? 'Country updated.'
          : t('countryCreated') ?? 'New country created.',
        timer: 1600,
        showConfirmButton: false,
      });
